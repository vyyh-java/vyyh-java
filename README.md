## Hi there 👋

### 👥 Group Projects

#### 1. [🤖 Moderation-bot](https://github.com/qy0909/moderation-bot.git)
* **Role**: Backend Developer
* **Highlights**: Engineered the core automated moderation logic and dynamic generative reply workflows.

#### 2. [📈 FinZen - Financial Management Webapp](https://github.com/vyyh-java/small-project.git)
* **Role**: Core Full-Stack Developer
* **Highlights**: 
  * Independently engineered the **Savings & Goal management module**, implementing secure full CRUD operations.
  * Developed an interactive data-analytic dashboard to track long-term savings goals and visual financial milestones.

#### 3. [🌱 Eco Challenge](https://github.com/vyyh-java/MAD_6.git)
* **Role**: UI/UX Designer, Front-End Developer & Full-Stack Developer (Learning-Material Module)
* **Highlights**: 
  * Crafted a high-fidelity mobile prototype themed around sustainability, featuring an immersive UI with refined gradients and glassmorphism styling.
  * Implemented structured **MVVM architecture** on the client side and applied seamless responsive viewport adjustments across devices.
* **🔗 Resources**:
  * 🎨 [Figma UI Prototype](https://www.figma.com/design/PbUd9Ka5kkXSeL5qNaYPwC/ecoapp?node-id=0-1&t=F2r1WcjWaqWp2gn6-1)
  * 🎬 [Demo Video (700MB HD)](https://drive.google.com/file/d/1JowNEJ6EdGsx_gNyW06IrozL-v1hdlZy/view?usp=drivesdk)
